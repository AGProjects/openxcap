...
class Storage(database.Storage):
    ...
    def _get_policy_from_xml(self, document):
        ...
        blocked_watchers = [w.attrib['id'].strip("sip:") for w in one]
        blocked_domains = [w.get('domain', '') for w in many]
        block = {'users': blocked_domains, 'domains': allowed_domains}
    ...
    def _update_allowed_watchers(self, trans, user, db_watchers, auth_policy):
        ...
        for w in to_add:
            ...
            try:
                ...
                print 'error while inserting watcher: %s@%s' % (w_username, w_domain)
    ...
    def _merge_watchers(self, trans, uri, document):
        ...
        #self._update_blocked_watchers(trans, user, db_terminated_watchers, xml_blocked_watchers)
